...
main(int argc, char **argv)
{
	...
	while (pcap_next_ex(handler, &header, &packet) >= 0) {
		...
		if (header->len != header->caplen) {
			printf("Warning!!! Capture size different "
				"than packet size: %u bytes\n",
				header->len);
			errCount++;
			fprintf(stderr, "C");
			continue;
		...
		if (nm_inject(txnmds[0], packet, header->caplen) == 0) {
	...
	printf("Packets # %i \t Error count: %i\n", packetCount, errCount);
